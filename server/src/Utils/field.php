...
function checkVar($var, $flags=0, $default=null){
    ...
    if(($flags & FIELD_SHA1) == FIELD_SHA1 && checkSHA1($var))
        $result = true;
    if(($flags & FIELD_IDENTIFIER) == FIELD_IDENTIFIER && checkID($var))
        $result = true;
...
function checkID($str){
	if(strpos($str, "_")){
		...
		$str = strtolower($str);
		$return = (in_array($str, array("true", "false", "1", "0", "wall", "barracks", "generator", "defender", "professor"), true));
	}
	return $return;
}