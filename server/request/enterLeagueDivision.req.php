...
class enterLeagueDivision{
    
    public function __request($player){
        
		if($player->character->league_group_id > 0 || $player->character->honor < 500)
			return Core::setError("errRemoveGameCurrencyNotEnough");
		
		$player->character->league_group_id = 100000;
		
		if(empty($player->character->league_opponents)){
		$player->character->league_opponents = '[1,2,3]';
		}
		
        $opp = DB::sql("SELECT `user_id` FROM `character` WHERE `league_group_id` <= {$player->character->league_group_id} AND `league_group_id` <= {$player->character->level} AND `level` >= 0 AND `id`<>{$player->character->id} ORDER BY `league_group_id` DESC LIMIT 3")->fetchALL(PDO::FETCH_NUM);
