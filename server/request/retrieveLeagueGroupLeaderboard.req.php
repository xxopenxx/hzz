    ...
    public function __request($player){
        ...
        if($sort_rank)
            ...
            if($character_name)
                ...
                $where = "`ch`.`league_group_id` LIKE '%{$league_group_id}%'";
            DB::sql("set @rank = 0");
            $centerRank = DB::sql("
                SELECT `ch`.`rank` FROM
                    (SELECT @rank := @rank+1 as 'rank', `id`, `name`, `league_group_id` FROM `character` ORDER BY `league_points` DESC) as ch
                WHERE {$where};
            ")->fetchColumn();
        ...
        $lb = DB::sql("
            SELECT `h`.* FROM
                (SELECT {$columns} FROM `character` ch LEFT JOIN `guild` g ON `g`.`id`=`ch`.`guild_id` ORDER BY `league_points` DESC) as h
