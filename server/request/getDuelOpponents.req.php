...
class getDuelOpponents{
    ...
    public function __request($player){
        ...
        $opp = DB::sql("SELECT `user_id` FROM `character` WHERE `honor` <= {$player->character->honor} AND `honor` >= 0 AND `level` <= {$player->character->level} AND `id`<>{$player->character->id} ORDER BY `honor` DESC LIMIT 10")->fetchALL(PDO::FETCH_NUM);
